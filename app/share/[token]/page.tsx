...
export default function SharedCharacterPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const [skillResult, itemResult, conditionResult] = await Promise.all([
        supabase.from("skills").select("*").eq("character_id", char.id).order("sort_order"),
        supabase.from("inventory_items").select("*").eq("character_id", char.id).order("created_at"),
        supabase.from("conditions").select("*").eq("character_id", char.id).order("created_at"),
      ]);
      setCharacter(char); setSkills(skillResult.data ?? []); setItems(itemResult.data ?? []); setConditions(conditionResult.data ?? []); setLoading(false);
    }
    load();
  }, [token]);
  ...
  return <main className="appShell wideShell"><div className="sharedHeader"><Link href="/" className="brandLink"><small>Public Character Record</small><strong>มหาคัมภีร์แห่งออดมา</strong></Link><span>โหมดดูอย่างเดียว</span></div><CharacterReadOnly character={character} skills={skills} items={items} conditions={conditions} /></main>;
}

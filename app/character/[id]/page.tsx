...
import CharacterExchange from "@/components/CharacterExchange";
...
export default function CharacterPage() {
    ...
    setItems(
      (itemResult.data ?? [])
        .map((item: any, index: number) => ({
          ...
          slot_index: Number.isInteger(item.slot_index)
            ? item.slot_index
            : index,
          allowed_equipment_slot:
            item.allowed_equipment_slot ??
  ...
  async function saveAll() {
    ...
    for (const condition of conditions) {
      const { error: conditionError } = await supabase
        .from("conditions")
        .upsert({ ...condition, character_id: id });
  ...
  async function uploadPortrait(event: ChangeEvent<HTMLInputElement>) {
    ...
    else setMessage("อัปโหลดภาพแล้ว");
  ...
  async function copyShareLink() {
    ...
    setMessage(
      character.is_public
        ? "คัดลอกลิงก์สาธารณะแล้ว"
        : "ต้องเปิดการแชร์สาธารณะและบันทึกก่อน"
    );
  }
  ...
  if (authLoading || loading) {
    return (
      <main className="appShell">
        <Loading text="กำลังเรียกหน้าต่างสเตตัส…" />
      </main>
    );
  }
    ...
    return (
      <main className="appShell">
        ...
        <div className="emptyPanel">
          ไม่พบตัวละคร หรือคุณไม่มีสิทธิ์เข้าถึง
        </div>
      </main>
    );
  }
    ...
    <main className="appShell wideShell">
      ...
      <div className="characterToolbar">
        ...
        <div className="navActions">
          ...
          {displayedView === "status" && canEdit ? (
            <button
              className="button"
              onClick={() => (editing ? saveAll() : setEditing(true))}
            >
              {editing ? "บันทึกทั้งหมด" : "แก้ไขตัวละคร"}
            </button>
          ) : null}
      ...
      <nav className="characterViewTabs" aria-label="เมนูตัวละคร">
        <button
          type="button"
          className={displayedView === "status" ? "active" : ""}
          aria-pressed={displayedView === "status"}
          onClick={() => changeView("status")}
        >
          <span>✧</span>
          <div>
            <small>Character</small>
            <strong>หน้าต่างสเตตัส</strong>
          </div>
        </button>
        ...
        {canViewInventory ? (
          <button
            type="button"
            className={displayedView === "inventory" ? "active" : ""}
            aria-pressed={displayedView === "inventory"}
            onClick={() => changeView("inventory")}
          >
            <span>▦</span>
            <div>
              <small>Inventory</small>
              <strong>ช่องเก็บของ</strong>
            </div>
            <b>
      ...
      {displayedView === "status" && !editing ? (
        <CharacterReadOnly
          ...
          actions={
            canEdit ? (
              <label className="button uploadButton">
                {uploading ? "กำลังอัปโหลด…" : "เปลี่ยนภาพตัวละคร"}
                <input
                  type="file"
                  accept="image/*"
                  hidden
      ...
      {displayedView === "status" && editing ? (
        <section className="editorLayout">
          <section className="panel editorPanel">
            ...
            <div className="editGrid">
              ...
              <Field label="คลาส">
                <input
                  ...
                  onChange={(event) =>
                    updateField("class_name", event.target.value)
                  }
                />
              </Field>
              <Field label="ระดับ">
              ...
              <Field label="เผ่าพันธุ์">
                <input
                  ...
                  onChange={(event) =>
                    updateField("race", event.target.value)
                  }
                />
              </Field>
              <Field label="ดาว">
                <input
                  ...
                  onChange={(event) =>
                    updateField("stars", event.target.value)
                  }
                />
              </Field>
              <Field label="สถานะ">
                <input
                  value={character.condition_text || ""}
                  onChange={(event) =>
                    updateField("condition_text", event.target.value)
                  }
                />
              </Field>
            </div>
              ...
              {STAT_KEYS.map((key) => (
                <Field label={key} key={key}>
                  <input
                    ...
                    onChange={(event) =>
                      updateStat(key, Number(event.target.value))
                    }
                  />
                </Field>
              ))}
            ...
            <Field label="จิ๊กซอว์ความทรงจำ">
              <textarea
                rows={3}
                value={character.memory || ""}
                onChange={(event) =>
                  updateField("memory", event.target.value)
                }
              />
            </Field>
            ...
            <label className="toggleLine">
              <input
                type="checkbox"
                checked={Boolean(character.is_public)}
                onChange={(event) =>
                  updateField("is_public", event.target.checked)
                }
              />
              <span>อนุญาตให้คนทั่วไปเปิดดูด้วยลิงก์แชร์</span>
            </label>
          </section>
          ...
          <section className="panel editorPanel">
            <div className="panelTitleRow">
              ...
              <button className="tinyButton" onClick={addSkill}>
                ＋ เพิ่ม
              </button>
            </div>
            ...
            <div className="editorCards">
              {skills.map((skill, index) => (
                <div className="editorCard" key={skill.id}>
                  <input
                    value={skill.name}
                    onChange={(event) =>
                      setSkills((rows) =>
                  ...
                  <input
                    ...
                    onChange={(event) =>
                      setSkills((rows) =>
                        rows.map((row, rowIndex) =>
                          rowIndex === index
                            ? { ...row, skill_type: event.target.value }
                            : row
                        )
                      )
                    }
                  ...
                  <input
                    ...
                    placeholder="มานา / คูลดาวน์"
                  />
                  <button
                    className="tinyButton danger"
                    onClick={() =>
                      removeRow("skills", skill.id, setSkills)
                    }
                  >
                    ลบ
                  </button>
          ...
          <section className="panel editorPanel">
            <div className="panelTitleRow">
              ...
              <button className="tinyButton" onClick={addCondition}>
                ＋ เพิ่ม
              </button>
            </div>
            ...
            <div className="editorCards">
              {conditions.map((condition, index) => (
                <div className="editorCard" key={condition.id}>
                  <input
                  ...
                  <button
                    className="tinyButton danger"
                    onClick={() =>
                      removeRow("conditions", condition.id, setConditions)
                    }
                  >
                    ลบ
                  </button>

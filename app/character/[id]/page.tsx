...
export default function CharacterPage() {
  ...
  const load = useCallback(async () => {
    ...
    const [characterResult, skillResult, itemResult, conditionResult] = await Promise.all([
  ...
  async function saveAll() {
    ...
    for (const item of items) {
      const { error: itemError } = await supabase.from("inventory_items").upsert({ ...item, character_id: id });
  ...
  function addItem() { setItems((current) => [...current, { id: crypto.randomUUID(), character_id: id, name: "ไอเทมใหม่", item_type: "ไอเทม", quantity: 1, description: "", equipped: false }]); }
  ...
  return (
    <main className="appShell wideShell">
      ...
      <div className="characterToolbar">
        <div>{message ? <span className="notice compact">{message}</span> : <span className="mutedText">เจ้าของตัวละครและ DM สามารถแก้ไขได้</span>}</div>
        <div className="navActions">
          {character.is_public ? <button className="button ghost" onClick={copyShareLink}>คัดลอกลิงก์แชร์</button> : null}
          {canEdit ? <button className="button" onClick={() => editing ? saveAll() : setEditing(true)}>{editing ? "บันทึกทั้งหมด" : "แก้ไขตัวละคร"}</button> : null}
          {editing ? <button className="button ghost" onClick={() => { setEditing(false); load(); }}>ยกเลิก</button> : null}
        </div>
      </div>

      {!editing ? (
        <CharacterReadOnly character={character} skills={skills} items={items} conditions={conditions} actions={canEdit ? <label className="button uploadButton">{uploading ? "กำลังอัปโหลด…" : "เปลี่ยนภาพตัวละคร"}<input type="file" accept="image/*" hidden onChange={uploadPortrait} disabled={uploading} /></label> : undefined} />
      ) : (
        <section className="editorLayout">
          ...
          <section className="panel editorPanel"><div className="panelTitleRow"><div><p className="eyebrow">Inventory</p><h2>คลังไอเทม</h2></div><button className="tinyButton" onClick={addItem}>＋ เพิ่ม</button></div><div className="editorCards">{items.map((item, index) => <div className="editorCard" key={item.id}><input value={item.name} onChange={(e) => setItems(rows => rows.map((row,i) => i===index ? {...row,name:e.target.value}:row))} /><input value={item.item_type || ""} onChange={(e) => setItems(rows => rows.map((row,i) => i===index ? {...row,item_type:e.target.value}:row))} placeholder="ประเภท" /><input type="number" min="0" value={item.quantity} onChange={(e) => setItems(rows => rows.map((row,i) => i===index ? {...row,quantity:Number(e.target.value)}:row))} /><textarea value={item.description || ""} onChange={(e) => setItems(rows => rows.map((row,i) => i===index ? {...row,description:e.target.value}:row))} placeholder="คำอธิบาย" /><label className="toggleLine"><input type="checkbox" checked={Boolean(item.equipped)} onChange={(e) => setItems(rows => rows.map((row,i) => i===index ? {...row,equipped:e.target.checked}:row))} /><span>สวมใส่</span></label><button className="tinyButton danger" onClick={() => removeRow("inventory_items", item.id, setItems)}>ลบ</button></div>)}</div></section>
